...
export const RecordItem = ({ data, onClick }: RecordItemProps) => {
  return (
    <div onClick={onClick} className="relative pl-8 pr-4 py-8 group cursor-pointer hover:bg-black/5 dark:hover:bg-white/5 transition-colors duration-200">
      <div className="absolute left-0 top-0 bottom-0 w-4 border-r border-concrete-300 dark:border-metal-700 flex flex-col items-center gap-4 py-4 bg-concrete-100 dark:bg-metal-900 transition-colors duration-300">
      ...
      <div className="border-l-2 border-transparent pl-4 group-hover:border-safety-orange transition-colors duration-300">
        <header className="flex items-baseline gap-4 mb-2 font-tech text-ink-500 dark:text-metal-500 text-sm">
          <span className="text-safety-dim group-hover:text-safety-orange transition-colors">#{data.serialNumber}</span>
          <span>[{data.date}]</span>
          <span className="flex-grow border-b border-concrete-300 dark:border-metal-800 border-dashed opacity-50"></span>
          <span className={`text-[10px] px-1 border ${data.mood === '波动' ? 'border-red-700 text-red-700 dark:border-red-900 dark:text-red-700' : 'border-concrete-300 dark:border-metal-700'}`}>状态: {data.mood}</span>
        </header>
        ...
        {data.image && (
          <div className="mt-4 border-2 border-concrete-300 dark:border-metal-700 p-1 bg-concrete-200 dark:bg-metal-900 inline-block">
             <img src={data.image} alt="证据" className="max-w-full h-20 w-auto object-cover opacity-80 dark:opacity-60 group-hover:opacity-100 transition-opacity contrast-125 sepia-[.2]" />
          </div>
        )}
      </div>
